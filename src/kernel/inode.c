...
bool remove_inode(uint32_t inode_no,struct partition*part)
{
    ...
    int roll_no = 0;
    struct inode_position inode_pos = locate_inode(part,inode_no);
    uint32_t sector_cnt = inode_pos.two_sector?2:1;
    char*io_buf = (char*)sys_malloc(SECTOR_SIZE * sector_cnt);
    if(io_buf == NULL){
        goto rollback;
    }
    readDisk(io_buf,part->my_disk,inode_pos.sector_lba,sector_cnt);
    memset(io_buf+inode_pos.block_offset,0,part->sb->inode_size); //内存中清除inode表中inode项
    //获取inode所有数据分布地址
