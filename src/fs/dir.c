...
bool remove_dir_entry(struct dir*parent_dir,uint32_t inode_no,struct partition*part)
{
    ...
    memcpy(addr_buf,parent_inode->i_sectors,12*sizeof(uint32_t));
    if(parent_inode->i_sectors[12]!=0) 
        readDisk(addr_buf+12,part->my_disk,parent_inode->i_sectors[12],1);
        ...
        if(addr_buf[addr_index]){ //若数据块存在
            ...
            bool found_tag = false;
                ...
                if(found_tag&&empty_cnt == dir_entry_cnt_per_sector - 1){
                    ...
                    sync_bitmap(BLOCK_BITMAP,part,addr_buf[addr_index]-part->sb->data_start_lba); //同步inode
                    //回收数据块
                    if(addr_index<12){
                        parent_inode->i_sectors[addr_index]=0;
                    }else{
